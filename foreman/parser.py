...
def parse_requires(
    requires: List[str], hardware: List[str]
) -> List[HardwareRequirement]:
    """Parse the 'requires' field into list of HardwareRequirement.

    Supports:
    - [all, inactive] -> all hardware must be at that state (expands to individual requirements)
    - [hardware_name, active] -> specific hardware must be at that state
    """
    if not requires or len(requires) != 2:
        return []

    target = requires[0]
    state = parse_state_string(requires[1])

    if target == 'all':
        return [
            HardwareRequirement(name=hw_name, state=state)
            for hw_name in hardware
        ]
    else:
        return [HardwareRequirement(name=target, state=state)]
...
def parse_yaml_file(file_path: Path) -> ParsedScenario:
    ...
    for ctrl_name, ctrl_config in controllers.items():
        ...
        reqs = parse_requires(requires, hardware)
    ...
    metadata = {}
    known_keys = {'controller_manager', 'transition_pause', 'hardware', 'controllers', 'goal_states'}
